...
const diffContextPerHunk = (file: PRFile, parser: AbstractParser) => {
	...
	hunks.forEach((hunk, idx) => {
		try {
			...
			console.log("getting enclosing context");
			const largestEnclosingFunction = parser.findEnclosingContext(
				updatedFile,
				lineStart,
				lineEnd
			).enclosingContext;
			console.log("finished getting enclosing context");

			if (largestEnclosingFunction) {
				console.log("setting scope range hunk map");
